...
export default class extends Vue {
    ...
    private async submitForm() {
      (this.$refs.postForm as Form).validate(async valid => {
        if (valid) {
          await this.createOrUpdataForm(
            this.postForm,
            'published',
            this.postForm.iscreate as boolean
          )
          this.loading = true
          this.$notify({
            title: 'Success',
    ...
    private async createOrUpdataForm(
      from: IArticleData,
      status: string,
      iscreate: boolean
    ) {
      if (iscreate) {
        return await this.createForm(from, status)
      } else {
        return await this.updataForm(from, status)
      }
    }
    ...
    private async createForm(from: IArticleData, status: string) {
      from.status = status
      from.platforms = JSON.stringify(from.platforms)
      const { data } = await createArticle({ article: from })
      console.log(data)
      this.postForm = data.article
      this.postForm.platforms = JSON.parse(this.postForm.platforms as string)
      const title = this.lang === 'zh' ? '编辑文章' : 'Edit Article'
